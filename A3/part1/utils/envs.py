...
def play_episode(env, policy, render = False, constraint = False):
    ...
    while not done:
        ...
        if constraint:
            rewards_c.append(info['cost'])
        if render: env.render()
        states.append(obs)
        rewards.append(reward)
    if constraint:
        return states, actions, rewards, rewards_c
    return states, actions, rewards

# Constrained CartPole-v0 Environment
# Based on: github.com/openai/gym/blob/master/gym/envs/classic_control/cartpole.py
class ConstrainedCartPole(gym.Env):
    ...
    def step(self, action):
        ...
        return np.array(self.state, dtype=np.float32), reward, done, trunc, {"cost": cost}
